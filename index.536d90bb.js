!function(){let e="https://api.musixmatch.com/ws/1.1/",t=document.querySelector(".search"),a=document.querySelector(".search__field"),s=document.querySelector(".results"),i=document.querySelector(".music"),r=document.getElementsByTagName("head")[0],n=document.querySelector(".pagination"),c=document.querySelector(".modal"),l=document.querySelector(".nav__btn--about-us"),o=document.querySelector(".modal__btn-close");console.log("8a497867c325bbcf4b4c9d286f8450bb");let m={search:{query:"",results:[],resultsView:[],perPage:10,page:1}},d=async function(e){try{let t=await fetch(e),a=await t.json();if(0===a.message.body.length)throw Error(`Status Code: ${a.message.header.status_code}`);if(0===a.message.header.available)throw Error("Not found Any Track!");return a}catch(e){throw e}},u=function(){let e=a.value;return a.value="",e},p=function(e){let t=`
  <div class="u-margin-top-big spinner"></div>
  `;e.innerHTML="",e.insertAdjacentHTML("afterbegin",t)},g=function(e,t="We couldn't find lyrics. try something else ..."){t.includes("is not valid JSON")&&(t="There is some problem with Genius Api. Try again later.");let a=`
          <div class="error">
            <div>
  ...
  `;e.innerHTML="",e.insertAdjacentHTML("afterbegin",a)},_=async function(){try{let t;let a=window.location.hash.slice(1);if(!a)return;console.log(a),p(i);let s=await d(`${e}track.lyrics.get?commontrack_id=${a}&apikey=${apiKey}`),n=await d(`${e}track.get?commontrack_id=${a}&apikey=${apiKey}`);m.search.results;let{lyrics:c}=s.message.body;console.log(c),c={id:c.lyrics_id,body:c.lyrics_body,script:c.script_tracking_url,imagePixel:c.pixel_tracking_url,copyright:c.lyrics_copyright,updatedTime:c.updated_time};let l=`<script type="text/javascript" src="${c.script}">`;r.insertAdjacentHTML("beforeend",l);let{track:o}=n.message.body;console.log(o),o={id:o.track_id,title:o.track_name,commontrackId:o.commontrack_id,hasLyrics:o.has_lyrics,explicit:o.explicit,favourite:o.num_favourite,albumId:o.album_id,albumName:o.album_name,artistId:o.artist_id,artistName:o.artist_name,updatedTime:o.updated_time,...0!==o.primary_genres.music_genre_list.length&&{genre:o.primary_genres.music_genre_list[0].music_genre.music_genre_name},genres:o.primary_genres.music_genre_list};let u=`https://api.genius.com/search?q=${encodeURIComponent(o.artistName+" "+o.title)}&access_token=${apiKeyGen}`,g=await fetch(u),_=await g.json();console.log(g),console.log(_);let h={};0!==_.response.hits.length&&(h=_.response.hits[0].result,console.log(h),h={headerThumbnail:h.header_image_thumbnail_url,headerImage:h.song_art_image_url,...h.release_date_components&&{releaseDate:h.release_date_components.year},artistImageHeader:h.primary_artist.header_image_url}),t=`
    ...
    `,i.innerHTML="",i.insertAdjacentHTML("afterbegin",t)}catch(e){console.error(`${e} \u{1F3C0}\u{1F3C0}\u{1F3C0}`),g(i,`${e}`)}},h=function(){let e=50/m.search.perPage,t=m.search.page,a=(e,a)=>`
